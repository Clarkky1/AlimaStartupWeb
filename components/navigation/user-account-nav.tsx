  ...
  return (
    ...
    >
      ...
      <div className="space-y-1 flex-1 overflow-hidden">
        ...
        {conversation.relatedConversations && conversation.relatedConversations.length > 0 && (
          <div className="mt-1">
            <span className="text-xs text-muted-foreground">
              +{conversation.relatedConversations.length} other service{conversation.relatedConversations.length > 1 ? 's' : ''}
            </span>
          </div>
        )}
      </div>
...
export function MessageCenter() {
  ...
  // Add useEffect to handle new messages
  useEffect(() => {
        ...
        return onSnapshot(q, (snapshot) => {
          ...
          if (preselectedConversationId) {
            ...
            if (targetConversation) {
              ...
              // Add this line to set the notification flag
  ...
  useEffect(() => {
    ...
    async function fetchMessages() {
      try {
        ...
        // Create query with AND condition containing OR filter
        if (!user) return;
        ...
        const unsubscribe = onSnapshot(q, (snapshot) => {
          ...
          // Check if this is coming from a notification
          const fromNotification = sessionStorage.getItem('fromNotification');
          if (fromNotification) {
            // Clear the flag and scroll with a delay to ensure rendering
  ...
  }, [selectedConversation, user, activeService, scrollToBottom]);
  ...
  const ServiceSelector = () => {
    if (!selectedConversation || !selectedConversation.relatedConversations) return null
    ...
    const allServices = [
      ...
      ...selectedConversation.relatedConversations.map((conv: any) => ({
        id: conv.serviceId,
        title: conv.serviceTitle
      }))
    ].filter(service => service.id) // Filter out any undefined services
    
    if (allServices.length <= 1) return null
    
    return (
      <div className="px-4 py-2 border-b">
        <div className="text-xs text-muted-foreground mb-1">Conversation Context:</div>
        <div className="flex flex-wrap gap-1">
  ...
  return (
    <div className="h-[calc(100vh-12rem)] overflow-hidden border rounded-lg bg-background">
      <div className="grid grid-cols-[260px_1fr] h-full">
                      ...
                      <div className="flex items-center space-x-3">
                        <div className="flex items-center gap-2">
                          ...
                          <div>
                            ...
                            {activeService && selectedConversation.serviceTitle && (
                              <p className="text-xs text-muted-foreground">
                                {activeService === selectedConversation.serviceId 
                                  ? `Re: ${selectedConversation.serviceTitle}` 
                                  : `Re: ${selectedConversation.relatedConversations?.find((c: any) => c.serviceId === activeService)?.serviceTitle || ''}`}
                              </p>
                            )}
                          </div>
                        </div>
        ...
        {/* Info Panel with independent scroll */}
        {selectedConversation && showUserInfo && (
          <div className="border-l w-[300px] flex flex-col h-full overflow-hidden">
            <div className="p-4 border-b bg-background">
              <div className="flex justify-between items-center">
                <h3 className="font-medium">Profile Information</h3>
                <Button variant="ghost" size="icon" onClick={() => setShowUserInfo(false)}>
                  <X className="h-4 w-4" />
                </Button>
              </div>
            </div>
